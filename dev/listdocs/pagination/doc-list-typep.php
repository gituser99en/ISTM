...
function doclisttypep($proj, $dtype, $tpeoffset){
...
if ($conn->connect_error) {
  ...
  if ($dtype==19){
  $sql1 = "SELECT * FROM document_id WHERE pid ='$proj' AND doctypeid =19 ORDER BY id LIMIT 10 OFFSET $tpeoffset";
...
while($row1 = mysqli_fetch_array($result1)){
 ...
 // echo' <br></br>';
  ...
  //  echo'<div>';
 // }
  $tptid = gettptid($last_id);
  echo'<a href="/dev/generate-doc/callpage.php?userid='.$userid.'&tpeoffset='.$tpeoffset.'&docidentifier='.$last_id.'&gid='.$gid.'&pid='.$pid.'&docname='.$name.'&doctype='.$dtid.'&tptid='.$tptid.'">';echo $name; echo'</a>';
   echo' <br></br>';
 // $ctr ++;
 // if($ctr == 10){
 //   $ctr=1;
